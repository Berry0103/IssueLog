...
function showDashIssueDetail(issueId) {
    fetch(`/api/issues/${issueId}`)
        ...
        .then(issue => {
            ...
            console.log(currentDashboardIssue);
            ...
            // 显示查看详情模态框
            if (!window.viewDashTransactionModalInstance) {
                window.viewDashTransactionModalInstance = new bootstrap.Modal(document.getElementById('viewDashTransactionModal'));
            }
            window.viewDashTransactionModalInstance.show();
        })
        .catch(error => {
...
function initDashUpdateEvents() {
    // 1. 确保模态框实例只创建一次
    if (!addDashUpdateModal) {
        const modalElement = document.getElementById('addDashUpdateModal');
        if (!modalElement) {
            console.error('子页面未找到 addDashUpdateModal 模态框元素');
            return;
        }
        addDashUpdateModal = new bootstrap.Modal(modalElement);
